...
colnames(FisherAdj)=c("ASD","ASD_SC","CA3_DGE","CA3_DOWN","CA3_UP","FMRP","ID","RBP","SYN","SZ_Loci","FromerEtAl2016","TF")
colnames(FisherOR)=c("ASD","ASD_SC","CA3_DGE","CA3_DOWN","CA3_UP","FMRP","ID","RBP","SYN","SZ_Loci","FromerEtAl2016","TF")
...
LabelMat[LabelMat == "0\n(1)"] <- NA
labeledHeatmap(Matrix = df, xLabels = colnames(df), yLabels = rownames(df), colorLabels =FALSE,colors=colorRampPalette(c("white", "red"))(50),textMatrix=LabelMat, setStdMargins = FALSE, cex.text = 0.5, main = paste("CA3 Modules GeneSets Enrichment"))
dev.off()
...
LabelMat[LabelMat == "0\n(1)"] <- NA
labeledHeatmap(Matrix = df, xLabels = colnames(df), yLabels = rownames(df), colorLabels =FALSE,colors=colorRampPalette(c("white", "red"))(50),textMatrix=LabelMat, setStdMargins = FALSE, cex.text = 0.5, main = paste("CA3 Modules Cell Type Enrichment"))
dev.off()
