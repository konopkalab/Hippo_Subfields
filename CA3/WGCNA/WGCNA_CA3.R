...
tmp$Batch <- as.numeric(as.factor(ifelse(tmp$ID %in% vec,"Batch1","Batch2")))
tmp$ID <- NULL
tmp$Subject_ID <- NULL
...
write.table(moduleColorsAutomatic, "CA3_colors.txt",sep="\t",quote=F)
save(net,file="CA3_NetData.RData")
...
rownames(kme)=NULL
write.table(kme,"KME_CA3.txt",sep="\t",quote=F)
...
write.table(modTraitCor,"modTraitCor_CA3.txt",sep="\t",quote=F)
modTraitP = corPvalueStudent(modTraitCor, nSamples)
write.table(modTraitP,"modTraitP_CA3.txt",sep="\t",quote=F)
...
write.table(MEs0, "CA3_Matrix_module_correlation.txt",sep="\t",quote=F)
...
write.table(moduleOutput, "ModuleOutput_CA3.txt", sep="\t", quote=F)
