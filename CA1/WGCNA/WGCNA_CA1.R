...
write.table(moduleColorsAutomatic, "CA1_colors.txt",sep="\t",quote=F)
save(net,file="CA1_NetData.RData")
...
rownames(kme)=NULL
write.table(kme,"KME_CA1.txt",sep="\t",quote=F)
...
write.table(modTraitCor,"modTraitCor_CA1.txt",sep="\t",quote=F)
modTraitP = corPvalueStudent(modTraitCor, nSamples)
write.table(modTraitP,"modTraitP_CA1.txt",sep="\t",quote=F)
...
write.table(MEs0, "CA1_Matrix_module_correlation.txt",sep="\t",quote=F)
...
write.table(moduleOutput, "ModuleOutput_CA1.txt", sep="\t", quote=F)
