...
pdf("CA1_Variance_Explained_DRUGS.pdf",width=4,height=4)
plotVarExp(var,"CA1 DRUGS VarExp")
dev.off()
...
vp <- varPart[order(varPart$Drugs, decreasing=TRUE),]

#' Plot Variance
plotPercentBars(vp[1:8,]) + 
theme_classic() +
ggsave("CA1_Top8genes_VarExp_DRUGS.pdf", width=6, height=5)

plotVarPart(vp) +
theme_classic() +
ggsave("CA1_AllGene_Explained_DRUGS.pdf", width=6, height=5)

# SVA calculation
mod = model.matrix(~Drugs+Sex+Age+PMI+RIN, data =pd)
...
pdf("CA1_DRUGS_Heatmap.pdf",width=6,height=6)
