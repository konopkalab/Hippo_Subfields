...
colnames(FisherAdj)=c("ASD","ASD_SC","DG_DGE","DG_DOWN","DG_UP","FMRP","ID","RBP","SYN","SZ_Loci","FromerEtAl2016","TF")
colnames(FisherOR)=c("ASD","ASD_SC","DG_DGE","DG_DOWN","DG_UP","FMRP","ID","RBP","SYN","SZ_Loci","FromerEtAl2016","TF")
...
LabelMat[LabelMat == "0\n(1)"] <- NA
labeledHeatmap(Matrix = df, xLabels = colnames(df), yLabels = rownames(df), colorLabels =FALSE,colors=colorRampPalette(c("white", "green"))(50),textMatrix=LabelMat, setStdMargins = FALSE, cex.text = 0.5, main = paste("DG Modules GeneSets Enrichment"))
dev.off()
...
LabelMat[LabelMat == "0\n(1)"] <- NA
labeledHeatmap(Matrix = df, xLabels = colnames(df), yLabels = rownames(df), colorLabels =FALSE,colors=colorRampPalette(c("white", "green"))(50),textMatrix=LabelMat, setStdMargins = FALSE, cex.text = 0.5, main = paste("DG Modules Cell Type Enrichment"))
dev.off()
