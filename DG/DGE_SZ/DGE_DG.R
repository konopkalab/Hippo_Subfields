...
varPart <- fitExtractVarPartModel(p, form, pd) # lmerControl(optimizer ="bobyqa")
vp <- varPart[order(varPart$Diagnosis, decreasing=TRUE),]

#' Plot Variance
plotPercentBars(vp[1:8,]) + 
theme_classic() +
ggsave("DG_Top8genes_VarExp.pdf", width=6, height=5)

plotVarPart(vp) +
theme_classic() +
ggsave("DG_AllGene_Explained.pdf", width=6, height=5)
...
pdf("DG_Heatmap.pdf",width=6,height=6)
